...
DECLARE
    ...
    v_product_kind TEXT;
BEGIN
    -- Validação de segurança: apenas o próprio usuário (ou via auth context) pode realizar o débito
    IF auth.uid() IS NULL OR auth.uid() <> p_user THEN
        RETURN jsonb_build_object('success', false, 'error', 'Unauthorized');
    END IF;

    -- Busca o ID interno do perfil e o saldo atual usando o user_id (auth)
    ...
    -- Registra a transação usando o v_profile_id (que é a FK esperada em transactions_user_id_fkey)
