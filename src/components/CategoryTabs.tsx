...
export default function CategoryTabs({ activeCategory, onSelect, categories: cats }: CategoryTabsProps) {
  ...
  return (
    <div className="bg-black/20 backdrop-blur-md border-b border-white/5 sticky top-[72px] sm:top-[88px] z-[90]">
        ...
        <div className="flex gap-0">
          {categories.map(cat => (
            <button
              ...
              className={`flex items-center gap-1.5 sm:gap-2 px-6 sm:px-8 py-4 sm:py-6 text-[11px] sm:text-[12px] font-bold uppercase tracking-[0.2em] border-b-2 transition-all duration-300 ${
                activeCategory === cat.key
                  ? 'text-white'
                  : 'text-white/20 hover:text-white/50 border-transparent'
              }`}
